        ...
        client.newCall(request).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                try {
                    ...
                    val today = Date()
                    var success = false
                    var message = "Invalid device/key"

                    val online = isOnline(activity)
                    ...
                    for (i in 0 until jsonArray.length()) {
                        ...
                        if (deviceId == allowedDeviceId && key == validKey) {
                            when {
                                today.after(expiryDate) -> message = "Key expired"
                                !allowOffline && !online -> message = "Activation not allowed offline"
                                else -> {
                                    success = true
